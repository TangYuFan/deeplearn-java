...
public class OnnxTensor extends OnnxTensorLike {
  ...
  public static OnnxTensor createTensor(OrtEnvironment env, FloatBuffer data, long[] shape, TensorInfo.OnnxTensorType type)
      throws OrtException {
  ...
  static OnnxTensor createTensor(
          OrtEnvironment env, OrtAllocator allocator, FloatBuffer data, long[] shape, TensorInfo.OnnxTensorType tensorType)
      throws OrtException {
    if (!allocator.isClosed()) {
      OnnxJavaType type = OnnxJavaType.FLOAT;
      return createTensor(type, allocator, data, shape,tensorType);
    } else {
      throw new IllegalStateException("Trying to create an OnnxTensor on a closed OrtAllocator.");
    }
  }
  ...
  private static OnnxTensor createTensor(
          OnnxJavaType type, OrtAllocator allocator, Buffer data, long[] shape, TensorInfo.OnnxTensorType tensorType) throws OrtException {
    OrtUtil.BufferTuple tuple = OrtUtil.prepareBuffer(data, type);
    TensorInfo info = TensorInfo.constructFromBuffer(tuple.data, shape, type);
    return new OnnxTensor(
            createTensorFromBuffer(
