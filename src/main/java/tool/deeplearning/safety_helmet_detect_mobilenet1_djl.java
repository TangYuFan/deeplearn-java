...
/**
 *   @desc : 安全帽检测 , mobilenet1.0 ,djl 推理
 *   @auth : tyf
 *   @date : 2022-06-15  09:11:41
 */
public class safety_helmet_detect_mobilenet1_djl {


    public static class MediumSafetyHelmetDetect {
    ...
    public static void main(String[] args) throws IOException, ModelException, TranslateException {
        ...
        String modelPath = new File("").getCanonicalPath()+
                "\\model\\deeplearning\\safety_helmet_detect_mobilenet1_djl\\mobilenet1.0.zip";

        // 测试图片
        String picPath = new File("").getCanonicalPath()+
                "\\model\\deeplearning\\safety_helmet_detect_mobilenet1_djl\\safety_helmet.jpg";



        Path imageFile = new File(picPath).toPath();
        Image image = ImageFactory.getInstance().fromFile(imageFile);
        Criteria<Image, DetectedObjects> criteria = new MediumSafetyHelmetDetect().criteria(image,modelPath);
