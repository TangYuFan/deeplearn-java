...
/**
*   @desc : BigGAN 图片生成,按照类别批量生成图片, djl 推理
 *
 *      使用的模型:
 *      https://mlrepo.djl.ai/model/cv/image_generation/ai/djl/pytorch/biggan-deep/0.0.1/biggan-deep-256.pt.gz
 *
*   @auth : tyf
*   @date : 2022-06-13  13:48:22
*/
public class big_gan_djl {



    public static class BigGANTranslatorFactory implements TranslatorFactory, Serializable {
    ...
    public static class BigGANTranslator implements NoBatchifyTranslator<int[], Image[]> {
    ...
    public static class BigGAN {

        private static final Logger logger = LoggerFactory.getLogger(BigGAN.class);

        String model;
        private BigGAN(String model) {
            this.model = model;
        }

        public Image[] generate(int[] input) throws IOException, ModelException, TranslateException {
            ...
            Criteria<int[], Image[]> criteria =
                    Criteria.builder()
                            ...
                            .optTranslator(new BigGANTranslator(0.5f))
                            .optFilter("size", "256")
            ...
            // 如果不从本地引入的话会自动根据 CV.IMAGE_GENERATION 注入模型,源码 PtModelZoo 如下
            // PtModelZoo() {
            ...
            //    }
            // 比如模型 biggan-deep 查看源码

            ZooModel<int[], Image[]> model = criteria.loadModel();
    ...
    public static void main(String[] args) throws ModelException, TranslateException, IOException {
        ...
        // https://mlrepo.djl.ai/model/cv/image_generation/ai/djl/pytorch/biggan-deep/0.0.1/biggan-deep-256.pt.gz
        // 模型保存位置
        // C:\Users\tyf\.djl.ai\cache\repo\model\cv\image_generation\ai\djl\pytorch\biggan-deep\24\256\imagenet\0.0.1
        // 这里通过本地引入 注意 optModelPath 和 optTranslator 需要一起设置,从源码中复制出来,打印model可以知道使用的 translator
        String model = new File("").getCanonicalPath()+
                "\\model\\deeplearning\\big_gan_djl\\biggan-deep-256.pt";


        // 需要生成的图片内容,这里生成五个图片（黑天鹅、金毛等）
        int[] input = {100, 207, 971, 970, 933};

        // 因为使用的是imagenet数据集,标签对应如下:
        // {
        //    "0": ["tench", "Tinca tinca"],
        ...
        // 图片生成,模型可以设置目标图片的尺寸
        BigGAN bigGAN = new BigGAN(model);
        Image[] generatedImages = bigGAN.generate(input);

        // 弹窗显示
        System.out.println("生成个数:"+generatedImages.length);
        Arrays.stream(generatedImages).forEach(n->{

            BufferedImage out = (BufferedImage)n.getWrappedImage();
