...
/**
 *   @desc : 人脸检测（ pcn） + 人脸年龄性别检测 + 关键点识别 + 人脸特征提取 + 人脸匹配
 *
 *
 *      模型 insightface：
 ...
 */
public class face_rec_det_insightface_pcn_2 {
    ...
    public static OrtEnvironment env6;
    public static OrtSession session6;

    public static OrtEnvironment env7;
    public static OrtSession session7;
    ...
    public static void init(String weight[]) throws Exception{
        ...
        env6 = OrtEnvironment.getEnvironment();
        session6 = env6.createSession(weight[5], new OrtSession.SessionOptions());

        env7 = OrtEnvironment.getEnvironment();
        session7 = env7.createSession(weight[6], new OrtSession.SessionOptions());
    ...
    public static void moduleInfo() throws Exception{
        ...
        System.out.println("---------模型[6]输入-----------");
        session6.getInputInfo().entrySet().stream().forEach(n->{
            String inputName = n.getKey();
            NodeInfo inputInfo = n.getValue();
            long[] shape = ((TensorInfo)inputInfo.getInfo()).getShape();
            String javaType = ((TensorInfo)inputInfo.getInfo()).type.toString();
        ...
        System.out.println("---------模型[6]输出-----------");
        session6.getOutputInfo().entrySet().stream().forEach(n->{
            String outputName = n.getKey();
            NodeInfo outputInfo = n.getValue();
            long[] shape = ((TensorInfo)outputInfo.getInfo()).getShape();
            String javaType = ((TensorInfo)outputInfo.getInfo()).type.toString();
        ...
        session6.getMetadata().getCustomMetadata().entrySet().forEach(n->{
            System.out.println("元数据:"+n.getKey()+","+n.getValue());
        });


        System.out.println("---------模型[7]输入-----------");
        session7.getInputInfo().entrySet().stream().forEach(n->{
            String inputName = n.getKey();
            NodeInfo inputInfo = n.getValue();
            long[] shape = ((TensorInfo)inputInfo.getInfo()).getShape();
            String javaType = ((TensorInfo)inputInfo.getInfo()).type.toString();
        ...
        System.out.println("---------模型[7]输出-----------");
        session7.getOutputInfo().entrySet().stream().forEach(n->{
            String outputName = n.getKey();
            NodeInfo outputInfo = n.getValue();
            long[] shape = ((TensorInfo)outputInfo.getInfo()).getShape();
            String javaType = ((TensorInfo)outputInfo.getInfo()).type.toString();
        ...
        session7.getMetadata().getCustomMetadata().entrySet().forEach(n->{
            System.out.println("元数据:"+n.getKey()+","+n.getValue());
        });
    }
    ...
    public static class FaceEmbed{
        ...
        // scrfd
        List<float[]> faceInfo_pcn; // 人脸检测
        List<Mat> face_pcn; // 人脸检测后对box扩展后截取的face
        List<List<float[]>> faceKeyPoint_pcn; // 对截取face进行关键点检测得到的关键点
        List<Mat> face_aligned_pcn;// 对截取的人脸和关键点进行对齐,对齐后是112*112
        List<Mat> face_hide_bg_pcn;// 对齐后的人脸进行人脸分割去掉背景,112*112
        List<String> faceAttribute_pcn;// 年龄和性别
        List<float[]> faceCode_pcn;// 特征提取
        ...
        public FaceEmbed(String path){
            ...
            // 集合初始化
            face_pcn = new ArrayList<>();
            faceInfo_pcn = new ArrayList<>();
            faceKeyPoint_pcn = new ArrayList<>();
            face_aligned_pcn = new ArrayList<>();
            face_hide_bg_pcn = new ArrayList<>();
            faceAttribute_pcn = new ArrayList<>();
            faceCode_pcn = new ArrayList<>();
        ...
        // 人脸检测 scrfd
        public void doDetection_pcn(float score_thres, float iou_thres){

            // 人脸检测

            // ---------模型[2]输入-----------
            // input_image -> [-1, 3, -1, -1] -> FLOAT
            // ---------模型[2]输出-----------
            // output_score -> [-1, 2, -1, -1] -> FLOAT
            // output_rotate -> [-1, 2, -1, -1] -> FLOAT
            // output_bbox -> [-1, 3, -1, -1] -> FLOAT

            // ---------模型[3]输入-----------
            // input_image -> [-1, 3, 24, 24] -> FLOAT
            // ---------模型[3]输出-----------
            // output_score -> [-1, 2] -> FLOAT
            // output_rotate -> [-1, 3] -> FLOAT
            // output_bbox -> [-1, 3] -> FLOAT

            // ---------模型[4]输入-----------
            // input_image -> [-1, 3, 48, 48] -> FLOAT
            // ---------模型[4]输出-----------
            // output_score -> [-1, 2] -> FLOAT
            // output_rotate -> [-1, 1] -> FLOAT
            // output_bbox -> [-1, 3] -> FLOAT

            // TODO
            // 推理参考:
            // https://github.com/DDogeee/PCN-pytorch-ONNX
            // https://github.com/richipower/PCN-opencv/blob/master/main.cpp

        }

        // 人脸关键点检测
        public void doKeyPointRec_pcn(){

            // 遍历每个人脸框
            face_pcn.stream().forEach(face->{
                ...
                try {
                    ...
                    OnnxTensor ten = OnnxTensor.createTensor(env5, array);
                    OrtSession.Result out = session5.run(Collections.singletonMap("data", ten));
                    // 解析 106关键点
        ...
        // 将人脸以外的背景部分进行屏蔽
        public void doHideFaceBg_pcn(float thres){

            // 遍历每个截取的人脸进行人脸实例分割,去掉背景
            // TODO 这个模型用于分割肖像对头发、手等人体信息依然进行了保留,后续可以替换换纯人脸的分割模型
            face_aligned_pcn.stream().forEach(face->{
                ...
                try {
                    ...
                    OnnxTensor tensor = OnnxTensor.createTensor(env7, FloatBuffer.wrap(chw), new long[]{1,3,192,192});
                    OrtSession.Result res = session7.run(Collections.singletonMap("x", tensor));
                ...
                face_hide_bg_pcn.add(resizeWithoutPadding(mat,112,112));
        ...
        public void doFaceAligned_pcn(){
            ...
            int count = face_pcn.size();
            for(int i=0;i<count;i++){
                Mat face = face_pcn.get(i);// 人脸矩阵
                // 人脸关键点,对齐可以5点对齐,也可以1067对齐,这里仅取5点对齐
                List<float[]> point = faceKeyPoint_pcn.get(i);
        ...
        // 人脸属性识别
        public void doFaceAttribute_pcn(){

            face_pcn.stream().forEach(n->{
                ...
                try {
                    ...
                    faceAttribute_pcn.add(gender+":"+age);
        ...
        // 人脸编码
        public void doFaceCode_pcn(){

            // 遍历每个人脸 112*112
            face_hide_bg_pcn.stream().forEach(face->{
                ...
                try {
                    ...
                    // 转为张量并推理
                    OnnxTensor ten = OnnxTensor.createTensor(env6, array);
                    OrtSession.Result out = session6.run(Collections.singletonMap("input.1", ten));
                    float[] embeds = ((float[][]) out.get(0).getValue())[0];
                    faceCode_pcn.add(embeds);
                }
                catch (Exception e){
                    e.printStackTrace();
                }
        ...
        public void show_pcn(){
            ...
            // 人脸个数
            int count = faceInfo_pcn.size();


            // 遍历人脸检测信息
            for (int i = 0; i < count; i++) {
                // 原始人脸框信息
                float[] n = faceInfo_pcn.get(i);
                float score = n[0];
                float x1 = n[1];
                float y1 = n[2];
                float x2 = n[3];
                ...
                // 年龄和性别
                String attribute = faceAttribute_pcn.get(i);
            ...
            for(int i=0;i<count;i++){
                ...
                // 人脸矩阵(对齐前的)
                Mat face = face_pcn.get(i);
                // 人脸关键点
                List<float[]> point = faceKeyPoint_pcn.get(i);
                // 人脸矩阵(对齐后的)
                Mat face_aligned = face_aligned_pcn.get(i);
                // 人脸矩阵(去掉背景)
                Mat face_hide_bg = face_hide_bg_pcn.get(i);
                // 特征向量
                float[] embed = faceCode_pcn.get(i);
                // 在对齐前的人脸上进行关键点标注
    ...
    public static void main(String[] args) throws Exception{
        ...
        String[] weights = new String[]{
                ...
                root + "detection_face_pcn\\pcn1_sd.onnx",// 人脸检测(效率低但旋转人脸检测高一些)
                root + "detection_face_pcn\\pcn2_sd.onnx",// 人脸检测(效率低但旋转人脸检测高一些)
                root + "detection_face_pcn\\pcn3_sd.onnx",// 人脸检测(效率低但旋转人脸检测高一些)
        ...
        // scrfd
        faceEmbed.doDetection_pcn(0.5f,0.7f);// 人脸检测,扩展人脸区域并截取
        faceEmbed.doFaceAttribute_pcn();// 人脸年龄、性别检测
        faceEmbed.doKeyPointRec_pcn();// 截取的人脸区域进行关键点检测,得到106点
        faceEmbed.doFaceAligned_pcn();// 截取的人脸区域进行5点对齐,并转为112*112,得到对齐后的人脸
        faceEmbed.doHideFaceBg_pcn(0.9f);// 对齐后的肖像分割去掉背景,得到最终需要进行编码的人脸 112*112
        faceEmbed.doFaceCode_pcn();// 人脸编码提取特征向量
        faceEmbed.show_pcn();// 弹窗显示
