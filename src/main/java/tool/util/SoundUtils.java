...
public class SoundUtils {
   ...
   * @throws Exception
   */
  public static void saveWavFile(NDArray wav, float volume, File outs,NDManager manager) throws Exception {
    System.out.println("manager.getDevice():"+manager.getDevice());
    Number va = _int16_max * volume;
    NDArray vb = NDArrays.maximum(0.01, wav.abs().max()).toDevice(manager.getDevice(),true);
    NDArray vc = wav.toDevice(manager.getDevice(),true);
    NDArray out = vc.mul(va).div(vb).toDevice(manager.getDevice(),true);
